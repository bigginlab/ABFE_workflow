...
def process_protein(protein_pdb_path, out_dir:str):
    ...
    subprocess.getoutput(f"{env_prefix}/bin/pdbfixer {protein_pdb_path} --output={pdb_fixed} --add-atoms=all --replace-nonstandard")  #TODO: dependency ?
    
    subprocess.getoutput(f"gmx pdb2gmx -f {pdb_fixed} -merge all -ff amber99sb-ildn -water tip3p -o {out_dir}/conf.gro -p {out_dir}/topol.top -i {out_dir}/posre.itp -ignh")
...
if __name__ == "__main__":
    ...
    prepare_input_files(protein_pdb=args.p, ligand_sdf=args.l, cofactor_sdf=args.c, out_dir=args.o)
